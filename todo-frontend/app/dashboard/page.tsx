...
import { getTodos, addTodo, updateTodo, deleteTodo, type Todo, getProfile } from '@/lib/api';
import TodoItem from '@/components/TodoItem';
import { AnimatePresence, motion } from 'framer-motion';
import { Plus, ListTodo, Search, Sparkles, AlarmClock, LogOut } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    getTodos()
      ...
      .catch((err) => {
        ...
        setError('Could not load todos. Check backend URL, port, and token.');
        setLoading(false);
      });
  }, [user]);
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                  {user ? `${user.name}'s workspace` : 'Personal workspace'}
                </p>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-2xl border border-input bg-card px-3 py-2 text-xs font-medium text-foreground shadow-sm hover:border-primary/40"
              >
                <LogOut size={14} />
                Log out
              </button>
            ...
            <div className="app-shell rounded-[26px] p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex items-center justify-between rounded-2xl bg-card px-4 py-3 shadow-sm">
                  ...
                  <span className="text-lg font-semibold text-foreground">{openCount}</span>
                </div>
                <div className="flex items-center justify-between rounded-2xl bg-card px-4 py-3 shadow-sm">
